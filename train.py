...
class train_network():
    ...
    def __init__(self, train_dl,valid_dl,sim_test_dl,k_n,EPOCH, ks, sig_length, LR, wdecay, device,root_path):
        self.k_n = k_n
        self.model = CNN1d(k_n = self.k_n,sig_seq=sig_length,k_s=ks)
        self.device = device
        self.root_dir = root_path
        self.model_dir = os.path.join(root_path,'savepoints')
        self.test_dir = os.path.join(root_path,'savedata')
        self.optimizer = torch.optim.Adam(self.model.parameters(), lr=LR, weight_decay=wdecay)
        self.loss = torch.nn.MSELoss()
        self.scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(self.optimizer,mode='min', factor = 0.7,verbose = True\
                                                                    , patience = 60000, cooldown = 1,eps = 1e-8)
        self.EPOCH = EPOCH
        self.histtrain = []
        self.histValid = []
        self.histTest = []
